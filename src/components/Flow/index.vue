<template>
  ...
  >
    ...
    <div class="lines-area" :class="[ toolFlag !== 0 ? 'cross' : '' ]" >
      <svg style="width: 100%;height: 100%">
        ...
        <g v-for="line in linesArry" @mousedown.stop="onPathSelected($event,line)" :key="line.id" :style="{zIndex: line.zIndex}">
          <path stroke-dasharray="10 10" class="line" :d="line.d" fill="none" :marker-end="`url(#${line.id})`"></path>
    ...
    <div ref="toolBar" class="tool-bar" :style="{ 
      ...
      <div @mousedown="onToolHandle" class="block">
        <div class="handle">
        </div>
      </div>
...
let toolHandleDown = ref(false)
let toolBar = ref(null)
let toolBarRect = ref({ x:150,y:50,ox:0,oy:0 })
const onToolHandle = (e) => {
  toolFlag.value = 0
  e.preventDefault()
  toolHandleDown.value = true
  toolBarRect.value.ox = e.clientX + (wrapper.scrollLeft - toolBarRect.value.x)
  toolBarRect.value.oy = e.clientY + (wrapper.scrollTop - toolBarRect.value.y)
}
...
const filterNodeInView = (arry,node) => {
  const nodes = arry.filter(v => !activityNodeIds.value.includes(v.id) && isInterect(v))
...
const onNodeResize = (params) => {
  ...
  switch(resizeInfo.type) {
    case '1':
      gudingPoing.x = resizeNode.x + resizeNode.width
      gudingPoing.y = resizeNode.y + resizeNode.height
      break;
    case '2':
      gudingPoing.x = resizeNode.x
      gudingPoing.y = resizeNode.y + resizeNode.height
      break;
    case '3':
      gudingPoing.x = resizeNode.x + resizeNode.width
      gudingPoing.y = resizeNode.y
      break;
    case '4':
      gudingPoing.x = resizeNode.x
      gudingPoing.y = resizeNode.y
   ...
   // 获取所有线条的位置比例信息
   resizeNode.outterLines.forEach(line => {
    if(line.startDir == 'top' || line.startDir == 'bottom') {
      targetNodeOutLineInfo[line.id] = {
        rate: (line.position.begin.x - resizeNode.x) / (resizeNode.width),
      }
    }else{
      targetNodeOutLineInfo[line.id] = {
        rate: (line.position.begin.y - resizeNode.y) / (resizeNode.height),
      }
    }
  })

  resizeNode.innerLines.forEach(line => {
    if(line.targetDir == 'top' || line.targetDir == 'bottom') {
      targetNodeInLineInfo[line.id] = {
        rate: (line.position.end.x - resizeNode.x) / (resizeNode.width),
      }
    }else{
      targetNodeInLineInfo[line.id] = {
        rate: (line.position.end.y - resizeNode.y) / (resizeNode.height),
      }
    }
  })

  console.log(targetNodeInLineInfo,'targetNodeInLineInfo')
  console.log(targetNodeOutLineInfo,'targetNodeOutLineInfo')
}

let targetNodeOutLineInfo = {}
let targetNodeInLineInfo = {}
// 按下node 活动的node重新设置zindex，防止拖动到高zindex的node上面时没有触发move
const onActivityNode = (params) => {
  const { nodes, offsetXY, nodeId } = params
  console.log(nodes.right > wrapper.scrollLeft,'node.right > wrapper.scrollLeft')
  ...
  allPoints = filterNodeInView(blockArry,nodes)
  offset = offsetXY
...
const move = (e) => {
  ...
  if(lineDown.value) {
    ...
    tempLine.begin = position.begin
    tempLine.end = position.end
    ...
    resizeNode.x = Math.min(gudingPoing.x,e.clientX + wrapper.scrollLeft) 
    resizeNode.y = Math.min(gudingPoing.y,e.clientY + wrapper.scrollTop)
    resizeNode.width = Math.abs(e.clientX + wrapper.scrollLeft - gudingPoing.x)
    resizeNode.height = Math.abs(e.clientY + wrapper.scrollTop - gudingPoing.y)
    let width =  resizeNode.width
    let height = resizeNode.height
    resizeNode.outterLines.forEach(line => {
      let { rate } = targetNodeOutLineInfo[line.id]
      let position = null, start = null
      switch (line.startDir) {
        case 'top':
          // 比例x, y
          start = { x: (rate * width) + resizeNode.x, y: resizeNode.y }
          position = drawLine(start,line.end,line.startDir,line.targetDir,true)
          break;
        case 'bottom':
          // 比例x, y
          start = { x: (rate * width) + resizeNode.x, y: resizeNode.y + height }
          position = drawLine(start,line.end,line.startDir,line.targetDir,true)
          break;
        case 'left':
          // 比例y, x
          start = { y: (rate * height) + resizeNode.y, x: resizeNode.x }
          position = drawLine(start,line.end,line.startDir,line.targetDir,true)
          break;
        case 'right':
          // 比例y, x
          start = { y: (rate * height) + resizeNode.y, x: resizeNode.x + width }
          position = drawLine(start,line.end,line.startDir,line.targetDir,true)
        break;
      }
      line.x = start.x
      line.y = start.y
      line.d = position.d
      line.begin = position.begin
      line.end = position.end
      line.position = position
    })
    resizeNode.innerLines.forEach(line => {
      let { rate } = targetNodeInLineInfo[line.id]
      let position = null, end = null
      switch (line.targetDir) {
        case 'top':
          // 比例x, y
          end = { x: (rate * width) + resizeNode.x, y: resizeNode.y }
          position = drawLine(line.begin,end,line.startDir,line.targetDir,true)
          break;
        case 'bottom':
          // 比例x, y
          end = { x: (rate * width) + resizeNode.x, y: resizeNode.y + height }
          position = drawLine(line.begin,end,line.startDir,line.targetDir,true)
          break;
        case 'left':
          // 比例y, x
          end = { y: (rate * height) + resizeNode.y, x: resizeNode.x }
          position = drawLine(line.begin,end,line.startDir,line.targetDir,true)
          break;
        case 'right':
          // 比例y, x
          end = { y: (rate * height) + resizeNode.y, x: resizeNode.x + width }
          position = drawLine(line.begin,end,line.startDir,line.targetDir,true)
        break;
      }
      line.d = position.d
  ...
  }else if(toolHandleDown.value) {
    // 加上滚动条滚动距离
    let _left = e.clientX + wrapper.scrollLeft - toolBarRect.value.ox
    let _top = e.clientY + wrapper.scrollTop - toolBarRect.value.oy

    let _h = linesArea.offsetHeight - toolBar.value.offsetHeight 
    let _w = linesArea.offsetWidth - toolBar.value.offsetWidth

    _left = Math.min(Math.max(0,_left),_w) // 左右边界
    _top = Math.min(Math.max(0,_top),_h)   // 上下边界

    toolBarRect.value.x = _left
    toolBarRect.value.y = _top
  }
}
...
const hanldeNodePosition = async (e,node,last) => {
  ...
  if(activityNode.length == 1) {
    for (let i = 0; i < allPoints.length; i++) {
      const alignment = checkAlignment(currentPoints, allPoints[i], 2);
      if (alignment.length) {
        let xarry = [], yarry = []
...
const onMouseup = (e) => {
  ...
  if(lineResize) {
    ...
    if(lineTarget) {
      if(lineResize == 1) {
        ...
        const node2 = findNode(tempLine.targetNode)
        if(node2) {
          const index = node2.innerLines.findIndex(line => line.id == tempLine.id)
          if(index !== -1) node2.innerLines.splice(index,1)
        }
  ...
  if(toolStart.value) {
    ...
    if(virtualRect.value.w && virtualRect.value.h) {
      addBlock(virtualRect.value)
    }
    
    virtualRect.value = { w: 0, h: 0, x: 0, y: 0  }
  }

  if(toolHandleDown.value) toolHandleDown.value = false
...
function addBlock(rect = {}) {
  const x = rect.x || getRandomInRange(130, 500)
  const y = rect.y || getRandomInRange(50, 150)
  const width = rect.w || getRandomInRange(100, 158)
  const height = rect.h || getRandomInRange(100, 120) 
  blockArry.push({
    id: 'node'+(+new Date()),
    left: x,
    top: y,
...
function isInterect(node) {  
  return node.right > wrapper.scrollLeft && node.left < wrapper.scrollLeft + wrapper.offsetWidth
